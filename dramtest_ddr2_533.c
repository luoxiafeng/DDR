...
{    
    ...
    writel_reverse(ADDR_UMCTL2_RFSHTMG       ,0x0041001c);//RFSHTMG,1066
    ...
    writel_reverse(ADDR_UMCTL2_INIT0         ,0x40020035);//[31:30] must be 2'b01
    ...
    //[31:16] mr,[15:0] emr
    writel_reverse(ADDR_UMCTL2_INIT3         ,0x0f730000);
    ...
    writel_reverse(ADDR_UMCTL2_DRAMTMG0      ,0x090c130c);//MEMC_FREQ_RATIO=2, [30:24] wr2pre= 8cycle + 15ns/2, [21:16] x16 tFAW=50ns(800)/2,[14:8] (t_ras_max(9xtREFI)-1)/2,tREFI=7.8us, [5:0] t_ras_min=37.5(800)/2
    //[6:0] t_rc 60ns(800),24ck
    writel_reverse(ADDR_UMCTL2_DRAMTMG1      ,0x00020310);//[20:16] t_xp/2, MR0[12] == 1'b0 ? tXPDLL(greater of 10tCK or 24ns) : tXP,[12:8] rd2pre tRTP = tAL+max(tRTP,4)(DDR3)/2,MEMC_FREQ_RATIO=2,[6:0] t_rc/2= 
    writel_reverse(ADDR_UMCTL2_DRAMTMG2      ,0x03040408);//[29:24], WL, mDDR,LPDDR2, DDR4 only,[21:16] RL/2 mDDR,LPDDR2, DDR4 only,[13:8] RL+BL/2+2-WL/2,[5:0] (CWL+BL/2+tWTR)/2
    //[17:12] t_mrd
...
writel_reverse(ADDR_UMCTL2_DRAMTMG4      ,0x04020304);//[28:24] tRCD-tAL/2, [19:16] tCCD/2, [11:8] tRRD/2,[4:0] (RoundUp(tRP/tCK)/2)+1
    ...
    //[31:16] mr,[15:0] emr
    writel_reverse(ADDR_UMCTL2_INIT3         ,0x0e730000);
...
writel_reverse(ADDR_UMCTL2_DBG0          ,0x00000016);
    ...
    //only for trigger release core_reset_n
...
void test_phy_reg_ddr2(void)
{
    ...
    writel_reverse(ADDR_PHY_MR0, 0x00000f73  );//1066,cl=7,wr=8
    read_trans_send( ADDR_PHY_MR0, read_tmp );
    writel_reverse(ADDR_PHY_MR0, 0x00000e73  );//1066,cl=7,wr=8
    read_trans_send( ADDR_PHY_MR0, read_tmp );
    ...
    writel_reverse(ADDR_PHY_DTPR0, 0x3ed87792);//1066
    ...
    writel_reverse(ADDR_PHY_DTPR1, 0x003800c0);
    ...
    writel_reverse(ADDR_PHY_DTPR2,   0x0641a8c8);//1066
    ...
    writel_reverse(ADDR_PHY_PTR1, 0x06b1a0ab );//sdram clock,[26:19] ='h90,(tDINIT1,tRFC+10ns) [18:0]='h411ab(tDINIT0,500us) 
    ...
    do
      read_trans_send(ADDR_PHY_PGSR,read_tmp);
    while((read_tmp & 0x0000000f) != 0x0000000f);
    ...
    do
      read_trans_send(ADDR_PHY_PGSR,read_tmp);
    while((read_tmp & 0x000003ff)!= 0x0000001f);//step 4
    writel_reverse(ADDR_UMCTL2_SWCTL         ,0x00000001);
